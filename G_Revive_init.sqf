...
G_Revive_AI_Incapacitated = [WEST,EAST]; //Array of sides of AI that will utilize revive system
G_Revive_Unit_Exclusion = []; //Array of variable names of units to exclude from the revive system
G_Revive_bleedoutTime = 180; //Amount of time (in seconds) unit is available to be revived, before being forced to respawn. If -1, no time limit.
...
G_Revive_actionTime = 5; //Time (in seconds) required for reviver to complete revive action
...
G_Revive_Reward = 1; //0 = No lives rewarded for revive. 1 and up = number of lives rewarded for reviving. (CAN be a decimal)
...
G_End_When_Side_Down = false; //true = When all units friendly to a side are incapacitated and/or unable to respawn the server will end the mission, false = server will not handle ending mission for side
...
G_Num_Respawns = -1; //Number of lives/respawns available to players (must be integer). -1 = unlimited, 0 and up are actual values.
...
G_Squad_Leader_Marker = true; //Displays marker on map indicating squad leader's position. true = enabled, false = disabled.
	...
	G_Squad_Leader_Mkr_Refresh = 5; //Time (in seconds) between refreshes in marker location. Must be a number greater than 0.
...
//Unit Tags
G_Unit_Tag = false; //Refers to unit "name tags" that display over unit's head on HUD. Only friendlies visible. true = enabled, false = disabled.
